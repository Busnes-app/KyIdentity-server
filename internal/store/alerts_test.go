...
func audited(t *testing.T, s *Store, action, actorName, targetID, targetType, outcome string, details string) {
	t.Helper()
	if err := s.RecordAuditEvent(&AuditEvent{ID: uuid.NewString(), ActorUsername: actorName, Action: action, TargetID: targetID, TargetType: targetType, Outcome: outcome, DetailsJSON: details, CreatedAt: time.Now().UTC()}); err != nil {
		t.Fatal(err)
	}
}
...
func TestLoginFailureThreshold(t *testing.T) {
	...
	fail := func(name string) {
		audited(t, s, "auth.login", name, "", "user", "failure", `{"reason":"user_not_found"}`)
	}
	fail("alice")
	fail("alice")
	fail("bob")
	...
	evaluate(t, s)
	got := openAlerts(t, s)
	if a, ok := got["login_failures/alice"]; !ok || a.Count != 3 || len(got) != 1 {
		t.Fatalf("threshold alert %+v", got)
	}
	fail("alice")
	evaluate(t, s)
	if a := openAlerts(t, s)["login_failures/alice"]; a.Count != 4 {
		t.Fatal("later failures should fold into the open alert", a)
	}
	// A burst evaluated in one pass opens once at the threshold and counts each failure
	// once; attacker-chosen usernames are display text, never header material.
	fail("evil\r\nBcc: x")
	fail("evil\r\nBcc: x")
	fail("evil\r\nBcc: x")
	evaluate(t, s)
	if a := openAlerts(t, s)["login_failures/evilBcc: x"]; a.Count != 3 {
		t.Fatal("burst counted more than once", a)
	}
	for k, a := range openAlerts(t, s) {
		if strings.ContainsAny(a.Title+a.Summary+a.Key, "\r\n") {
			t.Fatal("control characters in alert text", k, a)
		}
	}
}
...
func TestAlertDeliveriesRetryAndRecheckRecipients(t *testing.T) {
	...
	if n, err := s.DeliverAlerts(now, send); err != nil || n != 1 {
	...
	if n, _ := s.DeliverAlerts(now.Add(10*time.Second), send); n != 0 {
	...
	if n, err := s.DeliverAlerts(now.Add(time.Hour), send); err != nil || n != 1 {
		t.Fatal("retry", n, err)
	}
	a = openAlerts(t, s)["recovery_use/"+u.ID]
...
func TestAlertDeliveryGivesUpVisibly(t *testing.T) {
	...
	for i := 0; i < alertDeliveryAttempts; i++ {
		if n, err := s.DeliverAlerts(now, broken); err != nil || n != 1 {
			t.Fatal(i, n, err)
		}
		now = now.Add(24 * time.Hour)
	}
	if n, _ := s.DeliverAlerts(now, broken); n != 0 {
...
func TestOutageResolutionIsMailed(t *testing.T) {
	...
	if _, err := s.DeliverAlerts(time.Now().UTC(), send); err != nil {
		t.Fatal(err)
	}
